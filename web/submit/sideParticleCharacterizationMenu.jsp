...
<c:forEach var="charType" items="${allCharacterizationTypes}">
	...
	<c:choose>
		<c:when test="${canUserSubmit eq 'true' && charType.hasAction}">
			...
			&nbsp;<a href="deleteAction.do?charCategory=${charType.type}&dispatch=setup&particleType=${particleType}&particleName=${particleName}"><em>delete</em></a>
		</c:when>
	</c:choose>
	<br>
	<c:forEach var="aChar" items="${allCharacterizations[charType.type]}">		
		...
		<span class="indented${charType.indentLevel+1}"><a href="${url}" title="${aChar.name}">
				<font color="${aChar.viewColor}">${aChar.abbr}:${aChar.viewTitle}</font>
			</a></span>
		<br>
	</c:forEach>
	<br>
