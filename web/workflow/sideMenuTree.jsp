					...
					<table width="100%" height="95%" border="0" cellpadding="2" cellspacing="0">
						<tr>
							...
							<!--
								...
								<c:set var="runSeq" value="${assaySeq+totalAssayCount}"/>		
								<c:set var="aliquotSeq" value="${runSeq+totalRunCount*3+2}"/>
								<c:set var="inputFileSeq" value="${aliquotSeq+totalAliquotCount}"/>
								<c:set var="outFileSeq" value="${aliquotSeq+totalInputFileCount}"/>
													    		
								<c:forEach var="assayType" items="${allAssayTypes}" varStatus="assayTypeNum">
								    d.add(${assayTypeNum.count}, 0, '${assayType}','javascript:void(0)', '', '', '');									    

								    <c:forEach var="assay" items="${assayMap[assayType]}">
