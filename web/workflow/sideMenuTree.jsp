								    ...
								    <c:forEach var="assay" items="${sessionScope.workflow.assayBeanMap[assayType]}">
										...
										<c:forEach var="run" items="${assay.runBeans}">																
										    <c:set var="runSeq" value="${runSeq+1+2}"/>
  											...
  											d.add(${runSeq+1},${runSeq},'In','javascript:gotoPage(\'workflowForward.do?type=Input&method=setup&runId=${run.id}\')');
  											d.add(${runSeq+2},${runSeq},'Out','javascript:gotoPage(\'workflowForward.do?type=Output&method=setup&runId=${run.id}\')');
