								    ...
								    <c:forEach var="assay" items="${sessionScope.workflow.assayBeanMap[assayType]}">
										...
										<c:forEach var="run" items="${assay.runBeans}">																
										    <c:set var="runSeq" value="${runSeq+1+2}"/>
  											...
  											d.add(${runSeq+1},${runSeq},'In','javascript:gotoPage(\'workflowForward.do?menuType=in&assayType=${assay.assayType}&assayName=${assay.assayName}&runId=${run.id}&runName=${run.name}&inout=Input\')', '','', 'images/tree/folder.gif');
  											d.add(${runSeq+2},${runSeq},'Out','javascript:gotoPage(\'workflowForward.do?menuType=out&assayType=${assay.assayType}&assayName=${assay.assayName}&runId=${run.id}&runName=${run.name}&inout=Output\')', '','', 'images/tree/folder.gif');
