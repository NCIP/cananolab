...
import gov.nih.nci.cananolab.domain.characterization.physical.Size;
...
public class GridClientTest {
	...
	public int getNumberOfPublicData() {
		try {
			CQLQuery query = new CQLQuery();
			gov.nih.nci.cagrid.cqlquery.Object target = new gov.nih.nci.cagrid.cqlquery.Object();
	...
	public void testCQLFromFile() throws Exception {
		String fullClassName = "gov.nih.nci.cananolab.domain.characterization.physical.Size";
		CQLQuery query = (CQLQuery) gov.nih.nci.cagrid.common.Utils
				.deserializeObject(new FileReader(
						"c:/devel/caNanoLab/src/cql_example.xml"),
						CQLQuery.class);
		CQLQueryResults results = gridClient.query(query);
		results.setTargetClassname(fullClassName);
		CQLQueryResultsIterator iter = new CQLQueryResultsIterator(results);
		...
		try {
			if (!(args.length < 2)) {
				if (args[0].equals("-url")) {
					...
					test.testAll(args[1]);
					int sampleCount=test.getNumberOfPublicData();
					System.out.println("total data: "+sampleCount);
