...
public AliquotBean getAliquot(String aliquotId) throws Exception {
	...
	public ExecuteWorkflowBean getExecuteWorkflowBean() throws Exception {
		...
		Map<String, SortedSet<AssayBean>> assayTypeAssayMap = new HashMap<String, SortedSet<AssayBean>>();
		Map<String, SortedSet<RunBean>> assayRunMap = new HashMap<String, SortedSet<RunBean>>();
		Map<String, SortedSet<AliquotBean>> runAliquotMap = new HashMap<String, SortedSet<AliquotBean>>();
		Map<String, SortedSet<FileBean>> runInFileMap = new HashMap<String, SortedSet<FileBean>>();
		Map<String, SortedSet<FileBean>> runOutFileMap = new HashMap<String, SortedSet<FileBean>>();

		SortedSet<AssayBean> assays;
		SortedSet<RunBean> runs;
		SortedSet<AliquotBean> aliquots;
		...
		try {
			...
			for (Object obj : results) {
				...
				String statusStr = (items[3] == null) ? CalabConstants.ACTIVE_STATUS
						: CalabConstants.MASK_STATUS;
				if (items[2] instanceof Aliquot) {
					aliquotBean = new AliquotBean((Aliquot) items[2]);
					aliquotBean.setMaskStatus(statusStr);
				} else if (items[2] instanceof InputFile) {
					inFileBean = new FileBean((InputFile) items[2]);
					inFileBean.setFileMaskStatus(statusStr);
				} else if (items[2] instanceof OutputFile) {
					outFileBean = new FileBean((OutputFile) items[2]);
				...
				if (outFileBean != null) {
					if (runOutFileMap.get(runBean.getId()) == null) {
						outFiles = new TreeSet<FileBean>(
								new CalabComparators.FileBeanComparator());
						runOutFileMap.put(runBean.getId(), outFiles);
					} else {
			...
			workflowBean.setAssayTypeCount(assayTypeAssayMap.keySet().size());
			for (String assayType : assayTypeAssayMap.keySet()) {
				SortedSet<AssayBean> typeAssays = assayTypeAssayMap
						.get(assayType);
				assayCount += typeAssays.size();
			...
			workflowBean.setAssayBeanMap(assayTypeAssayMap);
		} catch (Exception e) {
			logger.error("Error in retriving execute workflow objects", e);
			throw new RuntimeException(
					"Error in retriving execute workflow objects ");
		} finally {
			ida.close();
		}
