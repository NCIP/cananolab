...
var app = angular.module('angularApp')
    ...
    $scope.characterizationTypeDropdownChanged = function() {
        ...
        delete $scope.characterizationData.assayType;
        delete $scope.characterizationData.characterizationName;
        if ($scope.characterizationData.characterizationType=='[other]') {
            alert("Other");
        }
        else {
            $scope.loader = true;
            $http({method: 'GET', url: '/caNanoLab/rest/characterization/getCharNamesByCharType?charType='+$scope.characterizationData.characterizationType}).
                success(function(data, status, headers, config) {
    ...
    $scope.characterizationNameDropdownChanged = function() {
        ...
        if ($scope.characterizationData.characterizationType=='[other]') {
            ...
            $scope.loader = true;
            $http({method: 'GET', url: '/caNanoLab/rest/characterization/getAssayTypesByCharName?charName='+ $scope.characterizationData.characterizationName}).
                success(function(data, status, headers, config) {
    ...
    $scope.getDomainFileUri = function() {
        for (var x = 0; x < $scope.data.protocolLookup.length;x++) {
            if ($scope.characterizationData.protocolBean.domain.id==$scope.data.protocolLookup[x].domainId) {
