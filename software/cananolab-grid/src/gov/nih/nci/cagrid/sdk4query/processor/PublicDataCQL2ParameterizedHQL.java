...
public class PublicDataCQL2ParameterizedHQL {
	...
	public PublicDataCQL2ParameterizedHQL(DomainTypesInformation typesInfo,
		...
		if (mods.isCountOnly()) {
			...
			if (mods.getDistinctAttribute() != null) {
				prepend.append("distinct ").append(secureAttribute)
						.append(", ").append(mods.getDistinctAttribute());
			} else {
				prepend.append("distinct ").append(secureAttribute);
			}
		} else {
	 ...
	 * @throws QueryProcessingException
	 */
	private void processTarget(Object target, StringBuilder hql,
			List<java.lang.Object> parameters, boolean avoidSubclasses)
			throws QueryProcessingException {
		...
		// start the query
		hql.append("From ").append(target.getName()).append(" as ").append(
				TARGET_ALIAS).append(' ');

		if (target.getAssociation() != null) {
			hql.append("where ");
			processAssociation(target.getAssociation(), hql, parameters,
					associationStack, target, TARGET_ALIAS);
		}
		if (target.getAttribute() != null) {
			hql.append("where ");
			processAttribute(target.getAttribute(), hql, parameters, target,
					TARGET_ALIAS);
		}
		if (target.getGroup() != null) {
			hql.append("where ");
			processGroup(target.getGroup(), hql, parameters, associationStack,
					target, TARGET_ALIAS);
		}

		if (avoidSubclasses) {
			boolean mustAddWhereClause = target.getAssociation() == null
